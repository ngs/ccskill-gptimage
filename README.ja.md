...
プロンプトを明示的に指定する必要がなく、プロジェクト内の情報やコンテキストを踏まえた最適なプロンプトを自動生成して、Claude Code のセッション内で ChatGPT Image 2.0 を使用して画像生成を行います。

- **🆕 API キー不要で使える** — ChatGPT サブスク + [Codex CLI](https://github.com/openai/codex) があれば、`OPENAI_API_KEY` 無しで gpt-image-2 を呼べる(`--backend codex`)
- **多言語テキスト描画** — 日本語(漢字/かな)・絵文字混在・縦書きまで含めて高品質に出力
- **Agentic 推論** — プロンプトから構造を計画して描画
- **参照画像で編集** — `--reference` で既存画像をベースに合成・部分修正
- **マスク編集 (inpainting)** — 既存画像の部分置換(`--backend api` 必須)
- **2 経路自動切替 (`--backend auto`)** — Codex 利用可能なら優先、失敗時 API へフォールバック
- **メタデータ自動保存** — プロンプト・revised_prompt・パラメータを JSON に記録
- **コスト最適化済みデフォルト** — 縦長 high が正方形 high より安い等の非自明な知見を組み込み

## 作例

本スキルを使って生成した画像の作例。プロンプトはスキルによる自動生成。全作例は [`docs/gallery.ja.md`](docs/gallery.ja.md) を参照。
...
<tr>
  ...
  <td align="center" width="33%"><a href="docs/gallery.ja.md"><img src="assets/capability-survey/categories/v2/18_nature_volcanic_coast_dawn.png" width="260" alt="フルギャラリーを見る"></a></td>
</tr>
<tr>
...
本スキルは 2 つの backend のどちらかで動作します。

**パターン A: ChatGPT 契約 + Codex CLI**
- Python 3.10 以上
- ChatGPT 契約 (Plus 以上)
- [Codex CLI](https://github.com/openai/codex) インストール済み + `codex login` でログイン済み
- → API キー不要、追加課金なし(サブスク枠内で使える)

**パターン B: OpenAI API キー**
- **Python 3.10 以上**
- **OpenAI API キー** + **Organization Verification 済み**(未検証 Org では 403)
- → 1 枚あたり数セント〜数十セントの従量課金

両方ある環境なら `--backend auto`(デフォルト)が Codex を優先 → 失敗時は API へ自動フォールバック。
...
GPT Image 2.0 を使えるようにします。2種類の方法があります。推奨はAです。

**A : Codex CLI を使う**

(すでにサブスク連携して Codex CLI を使用している場合は以下手順は不要です)
...
# ChatGPT アカウントでログイン
codex login
# 確認: tokens.access_token が入っていれば OK
cat ~/.codex/auth.json | python -c "import sys,json;print('OK' if json.load(sys.stdin).get('tokens',{}).get('access_token') else 'NG')"
```

**B : API キーを使う**

GPT Image 2.0 の使用には課金設定が必要です。以下を行って下さい。

- [OpenAI Platform](https://platform.openai.com/api-keys) にアクセスして API キーを発行
- Organization を Verified 状態にすること<br>(Settings → General → Verifications)を確認

### 3. APIキーを環境変数に設定(APIキーを使う場合のみ)
...
Claude Code で GPT Image 2.0 を使って画像生成するよう指示をするか、明示的に `/ccskill-gptimage` をプロンプトに入力して下さい。
...
Claude Code のスキルではなく、画像生成コマンドとして使用することもできます。

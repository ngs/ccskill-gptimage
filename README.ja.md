...
- **🆕 API キー不要で使える** — ChatGPT サブスク + [Codex CLI](https://github.com/openai/codex) があれば、`OPENAI_API_KEY` 無しで gpt-image-2 を呼べる(`--backend codex`)。35 枚 high quality 連続生成でもサブスク上限に到達しない実証あり([比較ドキュメント](docs/codex-vs-api-comparison.md))
...
本スキルは 2 つの backend のどちらかが利用可能なら動きます:

**パターン A: ChatGPT サブスク + Codex CLI(推奨・API キー不要)**
- **Python 3.10 以上**
- **ChatGPT サブスク契約**(Plus 以上)
- **[Codex CLI](https://github.com/openai/codex) インストール済み + `codex login` でログイン済み**
- → API キー不要、追加課金なし(サブスク枠内で使える)

**パターン B: OpenAI API キー(従来通り)**
- **Python 3.10 以上**

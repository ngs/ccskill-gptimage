...
**総合**: 改善 14 / 同等 16 / 微劣化 5 / 明確劣化 0
- **改善 + 同等で 30/35 = 85.7%**(Codex backend は API backend と同等以上の結果)
- **明確劣化はゼロ**(出力が完全に破綻したケース無し)
- **微劣化 5 件は全て UI モックアップ・インフォグラフィック・フローチャート系**(05/06/07/08)+ 19(プロンプト条件不一致の特殊ケース)

### Codex backend が劣化する系統的傾向

⚠️ となった 5 件のうち 19 を除く 4 件はすべて「**portrait/landscape 出力サイズに大きなキャンバスを使うべきジャンル**」(login UI、iOS 設定、infographic、flowchart):
...
### 19_urban_tokyo_rainy_night ⚠️*(注: プロンプト条件不一致)*
- **注意**: v2 比較対象は #015 の v2 リライト最終版(`19_v3` ファイル、強化された渋谷スクランブル交差点プロンプト)。v3-codex は git history の元 v2 弱化版プロンプトで生成。**プロンプト条件が同一ではないため厳密比較不可**
- **v2 (= v2 強化版)**: 渋谷スクランブル交差点、`カラオケ 747`/`居酒屋 2F・3F`/`龍角散`/`薬方` 等の本物感ある日本語看板群、雨上がりの濡れた路面、傘の人々、タクシー
- **v3-codex (元 v2 弱化版)**: 別の東京夜景、人混みは薄め、看板は読めない日本語、taxi tail light、濡れた路面。それでも東京夜景としては高品質
- **判定**: ⚠️ ただしこれは **プロンプトの強さ違いによる結果**で、Codex backend の問題ではない。同じ強化プロンプトで再生成すれば同等品質が出る可能性高
- **5 軸**: テキスト ⚠️(プロンプトが弱いので Codex も弱い結果)/ 指示順守 🟰 / 構図 🟰 / マテリアル 🟰 / 制約 🟰
...
| セル | 判定 | コメント |
|---|---|---|
| `low` | 🟰 | 両方とも `本と珈琲` ヘッダ + サブ + 女性読書シーン揃う。v3 微差で背景の cafe interior が豊か |
| `medium` | 🟰 | ほぼ完全同等。指示順守・文字描画・構図すべて拮抗 |
| `high` | 🟰 | 両方とも雑誌品質、v3 は光の暖かさが微妙にリード |

### 1024×1536 行(portrait)

| セル | 判定 | コメント |
|---|---|---|
| `low` | 🟰 | 両方ポートレート構図、ヘッダ正確、女性ポーズ自然。差は誤差範囲 |
| `medium` | 🟰 | 同等品質、v3 の窓辺ハイライトと枝の bokeh がやや繊細 |
| `high` | ✅ | v3 が**コーヒーカップ + 花瓶 + 椅子の彫りまで描き込み**、編集誌品質がさらに上 |

### 1536×1024 行(landscape)

| セル | 判定 | コメント |
|---|---|---|
| `low` | 🟰 | 両方ランドスケープ、ヘッダ + サブ + 主体揃う。微差 |
| `medium` | 🟰 | 同等、cafe 奥行き表現が拮抗 |
| `high` | 🟰 | v2 = v3、両方とも完成度高。微差で v3 が cafe 奥のオブジェクト(額装・植物・ペンダントライト)を明示 |

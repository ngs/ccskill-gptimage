...
`1024×1024 / high` — ドリップコーヒーを淹れる手元、桃・ノート・銅鍋、フィルム調
...
`1536×1024 / high` — 雨に濡れた路面のネオン反射、傘さす人混みのモーションブラー、漢字看板(カラオケ・もんじゃ等)まで自発描画。シネマティック teal-orange

<img src="../assets/capability-survey/categories/19_urban_tokyo_rainy_night.png" width="700" alt="Tokyo rainy night">

---

### 20. 乗り物(ヴィンテージカフェレーサー)

`1536×1024 / high` — 英国緑のカフェレーサー、ブラウンレザーシート、磨きこまれたクロームエキゾースト、夕日の石畳
...
`1024×1536 / high` ⭐(コスト有利) — オーバーハングを登るクライマー、夕陽の岩肌、ピーク瞬間捕捉
...
`1024×1024 / high` — 笑顔の柴犬、紅葉、石灯籠、ボケ味の温かい光
...
`1024×1024 / high` — 警戒の眼差し、凍える白い息、雪の毛皮、ワイルドライフフォト品質
...
`1536×1024 / high` — 濃墨の鷺、淡墨の水面と霧、和紙の風合い、朱印、東洋画の正統
...
- 実施日: 2026-04-23(初回 24 枚 + 追加 11 枚)
- 総枚数: **35 枚**(成功率 35/35 = **100%**、再生成ゼロ)
- 総コスト: 約 **$6.04 ≈ ¥910**(初回 $3.76 + 追加 $2.28)
- 並列実行: Tier 1 = 5 IPM 制約下で 7 バッチ × 約 2 分 = **約 50 分で全 35 枚完了**

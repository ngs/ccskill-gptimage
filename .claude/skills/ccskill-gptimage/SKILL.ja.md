...
- **原因が分からない時** → `ccskill-gptimage doctor` で環境 + backend を一括診断(秘密ファイルの中身は読まない。presence ベースで報告)
- **`ccskill-gptimage` の実行で毎回許可を聞かれる** → `~/.claude/settings.json` の `permissions.allow` に `Bash(ccskill-gptimage:*)` を追加(インストーラは設定を変更しません)

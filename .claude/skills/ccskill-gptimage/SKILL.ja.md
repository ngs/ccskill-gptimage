...
- 環境変数 `CCSKILL_GPTIMAGE_DIR` にこのスキルのリポジトリパスを設定
  ...
  ```
- 環境変数 `OPENAI_API_KEY` 設定(または `$CCSKILL_GPTIMAGE_DIR/.env` に記載)
- **OpenAI Organization Verification 済みであること**(未検証 Org では 403)
...
- **解像度**: Cookbook によれば gpt-image-2 は "any resolution per constraints" を受け付ける。本スキル CLI は現状 4 種(`auto / 1024x1024 / 1024x1536 / 1536x1024`)に制限。Cookbook は `1536x864`(16:9)も使う場面があり、近い代替として `1536x1024` を選ぶ
...
実証: Apple Business「紛失モード位置確認」スクリーンショットの地図部分を `--reference --quality high` 一発で匿名化(街路・河川・実在 POI を架空の住宅地に差し替え)しつつ、周辺 UI をデバイスシリアル番号レベルで保持した実例がある。`docs/dogfooding-log.md` 参照。

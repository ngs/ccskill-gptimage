...
- **Claude Code keeps asking permission to run `ccskill-gptimage`** → add `Bash(ccskill-gptimage:*)` to `permissions.allow` in `~/.claude/settings.json` (the installer intentionally does not modify your settings)
